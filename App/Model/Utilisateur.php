...
class Utilisateur extends BddConnect{
    ...
    private ?bool $active_utilisateur;
    private ?Roles $role;
    //Constructeur
    ...
    public function getActive(): ?bool {
        return $this->active_utilisateur;
    }
    public function setActive(?bool $active): void {
        $this->active_utilisateur = $active;
    }
    ...
    public function insertUtilisateur(): void {
        try {
            ...
            $requete = $this->connexion()->prepare('INSERT INTO utilisateur(
                nom_utilisateur,prenom_utilisateur, pseudo_utilisateur, mail_utilisateur,password_utilisateur,image_utilisateur,id_roles
                ) VALUE(?,?,?,?,?,?,?)'
            );
            $requete->bindParam(1,$nom,\PDO::PARAM_STR);
            $requete->bindParam(2,$prenom,\PDO::PARAM_STR);
            $requete->bindParam(3,$pseudo,\PDO::PARAM_STR);
            $requete->bindParam(4,$mail,\PDO::PARAM_STR);
            $requete->bindParam(5,$password,\PDO::PARAM_STR);
            $requete->bindParam(6,$image,\PDO::PARAM_STR);
            $requete->bindParam(7,$id_roles,\PDO::PARAM_INT);
            $requete->execute();
        } 
        catch (\Exception $e) {
    ...
    public function getUtilisateurByMail(): Utilisateur|bool{
        try {
            ...
            , pseudo_utilisateur,mail_utilisateur,password_utilisateur,image_utilisateur FROM utilisateur
    ...
    public function getUtilisateurByPseudo(): Utilisateur|bool{
        try {
            $pseudo = $this->pseudo_utilisateur;
            $requete = $this->connexion()->prepare('SELECT id_utilisateur,nom_utilisateur,prenom_utilisateur
            , pseudo_utilisateur,mail_utilisateur,password_utilisateur,image_utilisateur FROM utilisateur
    ...
    public static function getAllUtilisateur(): array
    {
        $bdd = new BddConnect();
        try {
            $selfId = $_SESSION["id"];
            $requete = $bdd->connexion()->prepare("SELECT id_utilisateur,nom_utilisateur,prenom_utilisateur, pseudo_utilisateur FROM utilisateur WHERE id_utilisateur != ?");
            $requete->bindParam(1,$selfId, \PDO::PARAM_INT);
            $requete->execute();
